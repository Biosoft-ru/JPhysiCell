    ...
    public void execute(Cell pCell, Phenotype phenotype, double dt) throws Exception
    {
        ...
        for( Cell pContactCell : neighbors )
        {
            ...
            if( pContactCell != pCell && pContactCell.phenotype.death.dead == false && pContactCell.type == CD8_Tcell_type
                    ...
                    && cell_cell_distance <= model.getParameterDouble( "epsilon_distance" ) * ( radius_mac + radius_test_cell ) )
            {
                phenotype.secretion.secretionRates[TNF_index] = 0;// (Adrianne) contact with CD8 T cell turns off TNF secretion
                break;
            }
            // (Adrianne) if it is not me, not dead and is a CD4 T cell that is within a very short distance from me, I will be able to phagocytose tumor (but not neccesarily dead) cells
                    ...
                    && cell_cell_distance <= model.getParameterDouble( "epsilon_distance" ) * ( radius_mac + radius_test_cell ) )
            {
                pCell.customData.set( "ability_to_phagocytose_cancer_cell", 1 ); // (Adrianne) contact with CD4 T cell induces macrophage's ability to phagocytose tumor cells
                break;
            }
        }
        ...
        if( pCell.customData.variables.get(time_to_next_phagocytosis_index).value > model.getCurrentTime() )
        {
            return;
        }
        ...
        double material_internalisation = pCell.customData.get( "material_internalisation" );
        ...
        for( Cell pTestCell : neighbors )
        {
            ...
            if( pTestCell != pCell && pTestCell.phenotype.death.dead == true && rng.UniformRandom() < probability_of_phagocytosis )
            {
                ...
                if( pTestCell.type == cancer_type )
                    System.out.println( " Macrophage (ID: " + pCell.ID + ") eats " + " dead cancer cell (ID: " + pTestCell.ID + ")" );
                ...
                pCell.customData.variables.get(time_to_next_phagocytosis_index).value = model.getCurrentTime() + time_to_ingest;
                ...
                // activate the cell
//                System.out.println( pCell.ID+" Activated" );
                phenotype.secretion.secretionRates[TNF_index] = pCell.customData.get( "activated_TNF_secretion_rate" );
                phenotype.secretion.saturationDensities[TNF_index] = 1;
                phenotype.secretion.uptakeRates[TNF_index] = 0.0;
                phenotype.motility.migrationSpeed = pCell.customData.get( "activated_speed" );
                pCell.customData.set( "activated_immune_cell", 1.0 );

                return;
            }
            else if( pTestCell != pCell && pCell.customData.get( "ability_to_phagocytose_cancer_cell" ) == 1
                    && rng.UniformRandom() < probability_of_phagocytosis ) // (Adrianne) macrophages that have been activated by T cells can phagocytose live cancer cells (hyperactivation)
            {
                // (Adrianne) obtain volume of cell to be ingested
                double volume_ingested_cell = pTestCell.phenotype.volume.total;
                ...
                if( pTestCell.type == cancer_type )
                    System.out.println(
                            " Hyperactivated macrophage (ID: " + pCell.ID + ") eats " + " cancer cell (ID: " + pTestCell.ID + ")" );
