...
//    private AgentColorer agentColorer = new AgentColorerDefault();
    ...
    public Visualizer2D setStubstrateIndex(int i)
    {
        this.substrateIndex = i;
        return this;
    }

    public Visualizer2D(String folder, String name, Section sec, double slice)
    {
        this.folder = folder;
        this.name = name;
        this.sec = sec;
...
//        setDefaultScheme();
    ...
    {
        Visualizer2D result = new Visualizer2D( folder, name, sec, slice );
...
//    }
    ...
    private BufferedImage draw(Microenvironment m, double slice, double time, String fileName) throws IOException
    {
        return draw( m, Section.Z, slice, time, fileName );
    }

    private BufferedImage draw(Microenvironment m, Section sec, double slice, double time) throws IOException
    {
        return draw( m, sec, slice, time, null );
    }

    private BufferedImage draw(Microenvironment m, Section sec, double slice, double time, String fileName) throws IOException
    {
        int extraWidth = 130;
        int textOffset = 10;
